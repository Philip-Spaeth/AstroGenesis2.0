...
void Disk::generateGasDisk(int start, int end)
{
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<> dist_theta(0.0, 2.0 * Constants::PI);
    std::uniform_real_distribution<> dist_uniform(0.0, 1.0);
    std::normal_distribution<> dist_z(0.0, g->z_Gas);
    std::normal_distribution<> dist_velocity(0.0, g->VelDis_Gas);

    double R_d = g->R_Gas / 5.0;  // Skalierungslänge der Scheibe
    double total_mass = g->M_Gas;
    double mass_per_particle = total_mass / static_cast<double>(g->N_Gas);
    ...
    for (int i = start; i < end; ++i)
    {
        ...
        } while (R > g->R_Gas); // Wiederhole, bis ein gültiger Radius gefunden wird
        ...
        double M_enc_disk = 0;
        if(g->N_Disk != 0) M_enc_disk = Exponential_Profile::enclosedMassExponential(R, g->M_Disk, g->R_Disk, 10);
        double M_enc_Gas = Exponential_Profile::enclosedMassExponential(R, g->M_Gas, g->R_Gas, 10);
        double v_theta = sqrt(Constants::G * (M_enc_disk + M_enc_bulge + M_enc_Halo + M_enc_Gas) / (R + 1e-10)); // Vermeide Division durch Null

        double v_r = dist_velocity(gen);
        double v_z = dist_velocity(gen) * (g->z_Gas / g->R_Gas);
